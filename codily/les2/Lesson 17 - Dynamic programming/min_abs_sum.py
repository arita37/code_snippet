...
have P ¬
S
2 ¬ Q and Q + P = S. The larger is P, the smaller is Q and the difference Q Q P.
...
def slow_min_abs_sum(A):
  ...
  for i in xrange(N):
    ...
    for j in range(N):  # all numbers
      for i in range(S, -1, -1):  # all sum possible 
        if (dp[i] == 1) and (i + A[j] <= S):  # current sum and next one ok
          dp[i + A[j]] = 1
    result = S

    
    for si in xrange(S // 2 + 1):
       if dp[i] == 1:
         result = min(result, S - 2 * si)
   return result
...
range 2..20 ✔OK
▶ extreme 
empty and single element ✔OK
▶ functional 
small functional test ✔OK
expand allPerformance tests
▶ medium1 
medium random ✔OK
▶ medium2 
multiples of 10 + 5 ✔OK
▶ big1 
multiples of 5 + 42 ✔OK
▶ big3 
all 4s and one 3 ✔OK
▶ big4 
multiples of 10 ✔OK
